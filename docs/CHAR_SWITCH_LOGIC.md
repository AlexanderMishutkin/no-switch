...
Defaults ship with the app so users can get started without manual configuration. Each preset targets a specific language or workflow and may include a mix of Latin and non-Latin characters.

- **German** – covers characters such as `A` ↔ `Ä`, `O` ↔ `Ö`, `U` ↔ `Ü`, `S` ↔ `ß`, and common punctuation variants (e.g., straight ↔ curly quotes).
- **Serbian** – combines Serbian Latin and Cyrillic needs. Chains map base letters to accented forms (e.g., `C` ↔ `Č` ↔ `Ć`) as well as cross-script transformations (e.g., `Н` ↔ `Nj`, `Љ` ↔ `Lj`), allowing rapid switching between scripts.
...
4. **Removing symbols** that are not useful for their workflow.
5. **Saving the preset** under a custom name for future reuse.

Custom presets live in the user configuration directory (exact path TBD) and override defaults when names collide.
...
- Custom presets are stored in a user-writable location. The app writes the entire preset (name + chains) after each modification.
