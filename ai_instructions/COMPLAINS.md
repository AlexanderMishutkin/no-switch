...
| 2025-10-19 | Missing `requirements.txt` prevented dependency installation and tests were not executed. | Added `requirements.txt`, switched the test suite to `unittest` (no external deps), and updated `ai_instructions/AGENTS.md` with the WSL virtualenv activation + pip usage instructions. |
| 2025-10-19 | Russian and Serbian presets lacked expected Cyrillic-trigger coverage (`bl`→`ы`, Cyrillic starters). | Expanded default presets to include symmetrical Latin/Cyrillic triggers, updated tests, and confirmed hotkey cycling behaviour. |
| 2025-10-19 | Removed nested test structure mirroring `src/char_switch_logic`. | Restored the `tests/char_switch_logic/` hierarchy with module-specific unittest suites and sys.path bootstrapping. |
