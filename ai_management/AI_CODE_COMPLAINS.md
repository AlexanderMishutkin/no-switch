...
| C2 | “What the file structure with poc.py?!??! It's not serious enough.” | User | Resolved | Expanded `docs/STRUCTURE.md` to explain the temporary role of `src/no_switch/poc.py` and future relocation plans. |
...
| C10 | “After splitting docs - go really fixing problems with your POC.” | User | Resolved | Added CLI diagnostics (`--list-chains`, `--demo`), better chain validation, and regression tests in `tests/test_poc.py`. |
